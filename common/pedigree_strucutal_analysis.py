...
def pedigree_strucutal_analysis(params):
    try:
        global config
        config = params

        target_df = pd.read_csv(params["param"]['target'], sep=params["sep"], low_memory=False)
        locus_df = pd.read_csv(params['locus_file'], sep=params["sep"], low_memory=False)

        df = locus_df[['locus', 'chr_id', 'position', 'HTP']].copy()

        for key, value in params["param"]['ancestors'].items():
            item_df = pd.read_csv(value, sep=params["sep"], low_memory=False)
            cdf = pd.merge(target_df, item_df, on='locus')
            cdf[key] = cdf.apply(lambda x: format_source(
                snp_compare(x['gt_x'], x['gt_y']), key), axis=1)
            df = pd.merge(
                df, cdf[['locus', key]], on='locus', how='left')

        df.sort_values(by=["chr_id", "position"], ascending=[True, True], inplace=True)

        df['source'] = df.apply(
            lambda x: get_certain_source(x.values.tolist()[-len(params["param"]['ancestors']):]), axis=1)
        ...
        if params["HTP"]:
            ...
            df.to_csv(
                f"{params['output']}/pedigree_structural_analysis.txt", 
                index=False,
                encoding="utf-8_sig",
                sep=params["sep"]
            )
